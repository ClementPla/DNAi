...
def preprocess(img, pixel_size=0.13, verbose=True, clarity=1.0):
    ...
    candidates = [
        (
            abs(k * max(1, round(target_product / k)) - target_product),
            max(1, round(target_product / k)),
            k,
        )
        for k in [3, 5]
    ]
    ...
    small = cv2.resize(
        img,
        None,
        fx=1 / best_downsample,
        fy=1 / best_downsample,
    ...
    if verbose:
        print(f"Background model built in {time() - start:.2f}s")
